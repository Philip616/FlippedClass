...
public class BulletinBoardChatInfo {
	int count=0;
	int check=0;
	String[][] courselist;
	String[][] str;
	public BulletinBoardChatInfo(String title,String week){
		DataFromDatabase MySQL_DB = new DataFromDatabase();
		MySQL_DB.Bulletin_BoardgetChatInfo(title, week);
			...
			try {
					...
					if(times<=10){
						...
						try {
								...
								courselist = new String[jsonArray.length()][6];
								for(int i = 0 ; i<jsonArray.length() ; i++)
								{
									...
									courselist[i][4] = jsonData.getString("student_ID");
									courselist[i][5] = jsonData.getString("note");
									Log.d("=====>", "RemindFragment 找到的Json = "+courselist[i][4]);
								...
								if(count>0){
									...
									{
										str[i][0] = courselist[i][4] ;//student_ID
										str[i][1] = courselist[i][3] ; //date
										str[i][2] = courselist[i][5] ;//note
									}
								}
								else{
									str = new String[1][3];
